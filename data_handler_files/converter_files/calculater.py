...
def calculate(info, team_header): # Az info tartalmazza az átküldött iterált csv_result sort, a team_header magáért beszél :)
	...
	if len(row_hlist) and len(row_alist) == 1: # Innentől kezdődik a tényleges számítás. "if":mindkét csapat csak a fejléccel rendelkezik.
		...
		pr_changing = (((int(info[8])-int(info[9]))-(fix_num[2]-float(row_alist[-1][8])))-fix_num[0])*fix_num[1]  # Az első "elif" csinálja a számítást, ha a hazai csapat érkezett újjonnan a bajnokságba
		xg_changing = (((float(info[10])-float(info[11]))-(fix_num[2]-float(row_alist[-1][8])))-fix_num[0])*fix_num[1]   # A második "elif" értelemszerüen ha a vendég csapat az új, vagyis nincs mit kiemelni a saját fájljából.
		prxg_changing = (((int(info[8])-int(info[9]))-(float(info[10])-float(info[11]))-(fix_num[2]-float(row_alist[-1][8])))-fix_num[0])*fix_num[1]   # Az "else" csinálja a "régi" csapatok számítását. Ekkor nem csak az iterált csv_resultot használja
		new_hpr = fix_num[2]+float(pr_changing)   # hanem a két csapat fájljaiból is ki tudja emelni az előző mérkőzésük változásait.
		old_apr = float(row_alist[-1][9])    # Természetesen csak az egyik feltétel fut le iterálásonként, mindegyik a maga "datas" adatait küldi vissza.
