...
def calculate(info, team_header):
    ...
    if len(row_hlist) and len(row_alist) == 1:
      ...
      pr_changing = (
                ((int(info[8])-int(info[9]))-(fix_num[2]-float(row_alist[-1][6])))-fix_num[0])*fix_num[1]
      xg_changing = (
                ((float(info[10])-float(info[11]))-(fix_num[2]-float(row_alist[-1][6])))-fix_num[0])*fix_num[1]
      prxg_changing = (((int(info[8])-int(info[9]))-(float(info[10])-float(info[11]))-(
                fix_num[2]-float(row_alist[-1][6])))-fix_num[0])*fix_num[1]
      new_hpr = fix_num[2]+float(pr_changing)
      ...
      pr_changing = (
                ((int(info[8])-int(info[9]))-(float(row_hlist[-1][6])-fix_num[2]))-fix_num[0])*fix_num[1]
      xg_changing = (
                ((float(info[10])-float(info[11]))-(float(row_hlist[-1][6])-fix_num[2]))-fix_num[0])*fix_num[1]
      prxg_changing = (((int(info[8])-int(info[9]))-(float(info[10])-float(info[11]))-(
                float(row_alist[-1][6])-fix_num[2]))-fix_num[0])*fix_num[1]
      ...
      new_aprxg = fix_num[2]-float(prxg_changing)
      datas = [new_hpr, new_apr, new_hxg, new_axg, new_hprxg, new_aprxg,
                old_hpr, fix_num[2], old_hxg, fix_num[2], old_hprxg, fix_num[2]]
      return (datas)
    else:
        ...
        xg_changing = ((float(info[10])-float(info[11]))-(float(row_hlist[-1][6])-float(
            row_alist[-1][6]))-fix_num[0])*fix_num[1]
        prxg_changing = ((int(info[8])-int(info[9]))-(float(info[10])-float(info[11]))-(
            float(row_hlist[-1][6])-float(row_alist[-1][6]))-fix_num[0])*fix_num[1]
        old_hpr = float(row_hlist[-1][7])
        new_hpr = old_hpr+float(pr_changing)
        old_apr = float(row_alist[-1][7])
        new_apr = old_apr-float(pr_changing)
